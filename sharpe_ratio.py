...
class Ticker:
    def __init__(self, ticker, years_back):
        ...
        self.data = pd.DataFrame(web.DataReader(
            self.ticker, 'yahoo', start_date, end_date))

        self.volatility, self.average_return, self.returns = self.volatility_averageReturn(
            years_back)

    def volatility_averageReturn(self, years_back):
        ...
        for date in interval_starts:
            while start_prices[i] == 0:
                try:
                    start_prices[i] = self.data.at[pd.Timestamp(
                        interval_starts[i]), 'Adj Close']
                except:
                    interval_starts[i] = interval_starts[i] + \
                        dt.timedelta(days=1)
        ...
        for date in interval_ends:
            while end_prices[i] == 0:
                try:
                    end_prices[i] = self.data.at[pd.Timestamp(
                        interval_ends[i]), 'Adj Close']
                except:
...
class SharpeSimulation:
    ...
    def simulate_portfolios(self):
        ...
        portfolios = [Portfolio(self.tickers, weight, self.years_back)
    ...
    def plot(self):
        ...
        expected_returns = [
            portfolio.ave_return for portfolio in self.portfolios]
        volatilities = [portfolio.volatility for portfolio in self.portfolios]

        plt.scatter(volatilities, expected_returns,
                    c=sharpe_ratios, marker='s')
...
if __name__ == '__main__':
    ...
    tickers = ['SPY', 'QQQ', 'DIA', 'TLT', 'VNQ', 'GLD']
    n = 10000
    years_back = 5
